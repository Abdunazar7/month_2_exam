...
route.post("/send-otp", async (req, res) => {
  try {
    ...
    const otpSecret = email + (process.env.OTP_SECRET_KEY || "default_secret_key");
...
route.post("/register", async (req, res) => {
    try {
        ...
        if (phoneExists) {
          return res.status(409).send({ error: "Customer with this phone number already exists!" });
        }
        
        const emailExists = await Customer.findOne({ email: value.email });
        if (emailExists) {
            return res.status(409).send({ error: "Customer with this email already exists!" });
        }
...
route.post("/login", async (req, res) => {
    try {
        ...
        res.status(200).send({ message: "Welcome!", customer });
      } catch (err) {
